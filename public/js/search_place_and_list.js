...
function search_public_list() {
    ...
    const grandpa_list_result = document.getElementsByClassName("search_list_and_place_result")[0];
    const list_data = {
    ...
    ajax("post", "/api/map_list/search/list", list_data, function (response_data) {
        ...
        if (response_data.length == 0) {
            const parent_list = document.createElement("p");
        ...
        for (let i = 0; i < response_data.length; i++) {
            ...
            const public_list_owner = document.createElement("p");
            ...
            const list_copy_number_img = document.createElement("img");
            ...
            const list_copy_number = document.createElement("p");
...
function search_place() {
    delete_child_element("search_list_and_place_result");
    const grandpa_place_result = document.getElementsByClassName("search_list_and_place_result")[0];
    ...
    ajax("post", "/api/map_list/search/place", place_data, function (response_data) {
        ...
        if (response_data.length == 0) {
            const parent_place = document.createElement("p");
        ...
        for (let i = 0; i < response_data.length; i++) {
            // 製作每一個 place 的內容
            const parent_place = document.createElement("div");
            ...
            const name_and_address_container = document.createElement("div");
            ...
            place_name.id =
                response_data[i].longitude + "_" + response_data[i].latitude + "_" + marker_order;
