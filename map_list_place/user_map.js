...
router.post("/list", async function (req, res) {
    ...
    const select_user_insert_list = await dao_map.select_2_conditions("user_map_list", "list_name", user_list_data.list_name, "user_name", user_list_data.user_name);
...
router.put("/list", async function (req, res) {
    ...
    const select_all_user_place = await dao_map.select("user_map_place", "user_name", user_list_data.user_name);

    res.send({
        success: "update OK",
        data: select_all_user_place
    });
});
...
router.put("/list/appear", async function (req, res) {
    ...
    // 將該清單的地點全部抓出來
    const select_user_update_place = await dao_map.select_2_conditions("user_map_place", "list_name", user_list_data.list_name, "user_name", user_list_data.user_name);

    // 尋找該清單的地點是否存在在其他清單中，並且要是 true 的狀態
    for (let i = 0; i < select_user_update_place.length; i++) {
        const select_place_is_exist_other_list = await dao_map.select_3_conditions("user_map_place", "place_name", select_user_update_place[i].place_name, "user_name", user_list_data.user_name, "appear_list", "true");

        select_user_update_place[i].place_is_exist = select_place_is_exist_other_list.length;
    }

    res.send({
        success: `update ${user_list_data.appear_list}`,
        data: select_user_update_place
    });
});
...
router.delete("/list", async function (req, res) {
    ...
    // 先找出清單是否真的存在
    const select_user_delete_list = await dao_map.select("user_map_list", "list_name", user_list_data.list_name);
    ...
    // 先將要刪除的清單所收藏的地點先找出來
    const select_user_delete_place = await dao_map.select_2_conditions("user_map_place", "list_name", user_list_data.list_name, "user_name", user_list_data.user_name);

    // 尋找該清單的地點是否存在在其他清單中，並且要是 true 的狀態
    for (let i = 0; i < select_user_delete_place.length; i++) {
        const select_place_is_exist_other_list = await dao_map.select_3_conditions("user_map_place", "place_name", select_user_delete_place[i].place_name, "user_name", user_list_data.user_name, "appear_list", "true");
...
router.post("/place", async function (req, res) {
    ...
    const select_user_insert_place = await dao_map.select_3_conditions("user_map_place", "user_name", user_list_data.user_name, "place_name", user_list_data.place_name, "list_name", user_list_data.list_name);
    ...
    const select_list_icon_and_appear = await dao_map.select_2_conditions("user_map_list", "user_name", user_list_data.user_name, "list_name", user_list_data.list_name);
    ...
    const select_map_place = await dao_map.select("map", "name", user_list_data.place_name);
    ...
    const last_place_id = await dao_map.select_last_insert_id();

    const select_user_last_place = await dao_map.select("user_map_place", "No", last_place_id[0]["LAST_INSERT_ID()"]);

    res.send({
        data: select_user_last_place
    });
});
...
router.post("/place/show", async function (req, res) {
    ...
    const select_user_list_place = await dao_map.select("user_map_place", "list_name", user_list_data.list_name);

    res.send({
        data: select_user_list_place
    });
});
...
router.delete("/place", async function (req, res) {
    ...
    // delete_user_list_name
    await dao_map.delete_3_conditions("user_map_place", "user_name", user_list_data.user_name, "list_name", user_list_data.list_name, "place_name", user_list_data.place_name, user_list_data.place_name);

    const check_place_is_exist = await dao_map.select_2_conditions("user_map_place", "user_name", user_list_data.user_name, "place_name", user_list_data.place_name);
