...
const leisure_farm_update_on_schedule = schedule.scheduleJob("* * * 1 */1 *", async function () {
    // 台北市資料大平台 臺北市休閒農場
    const taipei_leisure_farm = {
        url:
            "https://data.taipei/opendata/datalist/apiAccess?scope=resourceAquire&rid=2bbc419c-d774-4bcf-812b-b87b6fd15abb",
        category: "休閒農場",
        place_icon: "farm",
        place_name: "農場名稱",
        place_address: "地址",
    ...
    const attractions_leisure_farm = {
    ...
    // 政府資料開放平台 休閒農業 更新接在臺北休閒農場後面
    const outdoor_edu_leisure_farm = {
    ...
    // 政府資料開放平台 休閒農業 更新接在臺北休閒農場後面
    const permit_agri_leisure_farm = {
...
const health_update_on_schedule = schedule.scheduleJob("* * * 3 */1 *", async function () {
    // 台北市資料大平台 臺北市居家護理所
    const nursing_facility = {
        url:
            "https://data.taipei/opendata/datalist/apiAccess?scope=resourceAquire&rid=77b20322-618d-4a1e-aaaa-bf42a363dcc9",
        category: "護理所",
        place_icon: "therapy",
        place_name: "機構名稱",
        place_address: "地址",
    ...
    // 台北市資料大平台 臺北市精神復健機構
    const rehabilitation_institute = {
        url:
            "https://data.taipei/opendata/datalist/apiAccess?scope=resourceAquire&rid=7ece5203-926a-47a6-a426-2599f1beded1",
        category: "復健",
    ...
    const inoculation_hotel = {
        url:
            "https://data.taipei/opendata/datalist/apiAccess?scope=resourceAquire&rid=3063803c-8794-4d19-ab1c-3e602dd77506",
        category: "診所",
        place_icon: "clinic",
        place_name: "title",
    ...
    // 台北市資料大平台 高度近視合約醫院
    const myopia_hospital = {
        url:
            "https://data.taipei/opendata/datalist/apiAccess?scope=resourceAquire&rid=29fe058f-8ff8-48c2-b4b1-9948235d90db",
        category: "眼科",
        location: "地址",
        taipei_city: "臺北市",
        place_icon: "eye",
    ...
    // 台北市資料大平台 戒菸合約醫事機構
    let drug_dentist_clinic_hospital_request = await crawler.normal_request(
        "https://data.taipei/opendata/datalist/apiAccess?scope=resourceAquire&rid=43f32f04-e57f-47f5-9658-2494311c0f86"
    );
    ...
    axios
        .get("http://tada2002.ehosting.com.tw/Support.Tada2002.org.tw/NewsDtl.aspx?pk=104")
        .then(function (response) {
            const $ = cheerio.load(response.data); // 載入 body
...
const school_update_on_schedule = schedule.scheduleJob("* * * 4 */1 *", async function () {
    // 維基百科 普通型高級中等學校
    axios
        .get(
            "https://zh.wikipedia.org/wiki/%E4%B8%AD%E8%8F%AF%E6%B0%91%E5%9C%8B%E6%99%AE%E9%80%9A%E5%9E%8B%E9%AB%98%E7%B4%9A%E4%B8%AD%E7%AD%89%E5%AD%B8%E6%A0%A1%E5%88%97%E8%A1%A8#%E8%87%BA%E5%8C%97%E5%B8%82"
        )
        .then(function (response) {
            const $ = cheerio.load(response.data); // 載入 body
    ...
    axios
        .get(
            "https://zh.wikipedia.org/wiki/%E4%B8%AD%E8%8F%AF%E6%B0%91%E5%9C%8B%E6%8A%80%E8%A1%93%E5%9E%8B%E9%AB%98%E7%B4%9A%E4%B8%AD%E7%AD%89%E5%AD%B8%E6%A0%A1%E5%88%97%E8%A1%A8#%E8%87%BA%E5%8C%97%E5%B8%82"
        )
        .then(function (response) {
            const $ = cheerio.load(response.data); // 載入 body
    ...
    axios
        .get(
            "https://zh.wikipedia.org/wiki/%E5%8F%B0%E7%81%A3%E5%AE%97%E6%95%99%E5%AD%B8%E6%A0%A1%E5%88%97%E8%A1%A8#%E8%87%BA%E5%8C%97%E5%B8%82"
        )
        .then(function (response) {
            const $ = cheerio.load(response.data); // 載入 body
    ...
    axios
        .get(
            "https://zh.wikipedia.org/wiki/%E8%87%BA%E5%8C%97%E5%B8%82%E5%9C%8B%E6%B0%91%E4%B8%AD%E5%AD%B8%E5%88%97%E8%A1%A8"
        )
        .then(function (response) {
            const $ = cheerio.load(response.data); // 載入 body
...
const living_update_on_schedule = schedule.scheduleJob("* * * 25 */1 *", async function () {
    ...
    hotel_and_homestay_request = JSON.parse(hotel_and_homestay_request.data.substring(1)).XML_Head
        .Infos.Info;
    ...
    // 台北市資料大平台 環保旅店
    const environmental_hotel = {
        url:
            "https://data.taipei/opendata/datalist/apiAccess?scope=resourceAquire&rid=adef2044-760f-40bd-8e13-a7fda6d011de",
        category: "環保旅店",
        place_icon: "lodging",
        place_name: "名稱",
        place_address: "地址",
...
const other_update_on_schedule = schedule.scheduleJob("* * * 26 */1 *", async function () {
    ...
    const moto_charge = {
        url:
            "https://data.taipei/opendata/datalist/apiAccess?scope=resourceAquire&rid=c2b666e0-f848-4609-90cb-5e416435b93a",
        category: "電動機車充電",
        place_icon: "charging",
        place_name: "單位",
        place_address: "地址",
    ...
    const recycle_community = {
        url:
            "https://data.taipei/opendata/datalist/apiAccess?scope=resourceAquire&rid=0ab06b17-3ac0-4a7b-9e64-66ef69bba697",
        category: "社區資源回收站",
        place_icon: "recycle",
        place_name: "回收站名稱",
        place_address: "地址",
    ...
    // 台北市資料大平台 臺北市廢棄物處理場
    const waste_disposal_site = {
        url:
            "https://data.taipei/opendata/datalist/apiAccess?scope=resourceAquire&rid=13ac57a2-f4a1-4e9b-9372-9ce1c7f85df0",
        category: "廢棄物處理場",
        place_icon: "other",
    ...
    const towing_field = {
        url:
            "https://data.taipei/opendata/datalist/apiAccess?scope=resourceAquire&rid=24045907-b7c3-4351-b0b8-b93a54b55367",
        category: "拖吊場",
        place_icon: "other",
        place_name: "拖吊保管場名稱",
        place_address: "地址",
    ...
    let video_arcade_request = await crawler.normal_request(
        "https://data.taipei/opendata/datalist/apiAccess?scope=resourceAquire&rid=6a95357f-e76a-4cc0-84f6-27e550ced5e5"
    );
    ...
    const district_office = {
        url:
            "https://data.taipei/opendata/datalist/apiAccess?scope=resourceAquire&rid=a0484907-e5ce-4d5b-ac4a-42c1e7684326",
        category: "區公所",
    ...
    // 中華郵政 臺北市郵局
    axios
        .get("https://www.post.gov.tw/post/internet/I_location/index_all.jsp")
        .then(function (response) {
            const $ = cheerio.load(response.data); // 載入 body
...
const play_on_schedule = schedule.scheduleJob("* * * 27 */1 *", async function () {
    ...
    for (let i = 0; i < culter_asset_request.length; i++) {
        const address_and_name_data = {
            ...
            place_address:
                culter_asset_request[i].belong_city_name + culter_asset_request[i].belong_address,
    ...
    // 台北市資料大平台 藝文館所
    const art_and_culture_hall = {
        url:
            "https://data.taipei/opendata/datalist/apiAccess?scope=resourceAquire&rid=dfad2ec4-fa19-4b2f-9efb-f6fe3456f469",
        category: "藝文館所",
    ...
    // 台北市資料大平台 臺北市環境教育機構
    const environmental_education = {
        url:
            "https://data.taipei/opendata/datalist/apiAccess?scope=resourceAquire&rid=9d6b643e-5d71-46e7-8368-eb0aaf907171",
        category: "環境教育機構",
    ...
    // 政府資料開放平台 博物館
    let museum_request = await crawler.normal_request(
    ...
    axios
        .get(
            "https://zh.wikipedia.org/wiki/%E8%87%BA%E7%81%A3%E5%A4%9C%E5%B8%82%E5%88%97%E8%A1%A8#%E8%87%BA%E5%8C%97%E5%B8%82"
        )
        .then(function (response) {
            const $ = cheerio.load(response.data); // 載入 body
    ...
    axios
        .get(
            "https://zh.wikipedia.org/wiki/%E5%8F%B0%E7%81%A3%E9%9B%BB%E5%BD%B1%E9%99%A2%E5%88%97%E8%A1%A8"
        )
        .then(function (response) {
            const $ = cheerio.load(response.data); // 載入 body
