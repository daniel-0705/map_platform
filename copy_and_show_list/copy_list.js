...
router.post("/", async function (req, res) {
    ...
    mysql.con.beginTransaction(async function () {
        try {
            const select_public_list_result = await dao_map.select("user_map_list", "list_id", list_data.list_id);

            const insert_copy_list = {
                category: "true",
            ...
            // 先確認使用者清單沒有與複製清單名稱重複
            const select_copy_list_in_user = await dao_map.select_2_conditions("user_map_list", "user_name", insert_copy_list.user_name, "list_name", insert_copy_list.list_name);
            ...
            // 搜尋被複製的清單裡的地點
            const select_place_in_copy_list = await dao_map.select_2_conditions("user_map_place", "user_name", select_public_list_result[0].user_name, "list_name", list_data.list_name);

            // 新增複製的地點到使用者中
            for (let i = 0; i < select_place_in_copy_list.length; i++) {
                const insert_copy_places = {
                    user_name: user_data.name,
                    list_name: `${list_data.list_name}(複製)`,
            ...
            // 更新被複製名單的複製數量
            const select_owner_list = await dao_map.select_2_conditions("user_map_list", "list_id", list_data.list_id, "list_name", list_data.list_name);
            ...
            // update_owner_list
            await dao_map.update(
                "user_map_list",
                "list_id",
                select_owner_list[0].list_id,
                update_owner,
                update_owner.list_id
            );

            // 將使用者的複製清單裡面的地點找出來傳給前端
            const select_place_of_copy_list = await dao_map.select_2_conditions("user_map_place", "user_name", insert_copy_list.user_name, "list_name", insert_copy_list.list_name);
            ...
            res.send({
                success: "copy OK",
                data: select_place_of_copy_list
            });
        } catch (err) {
