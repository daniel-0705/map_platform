...
function render_user_all_lists_name() {
    ...
    if (localStorage.getItem("access_token")) {
        ...
        ajax("post", "/api/map_list/user/result", list_data, function (response_data) {
            ...
            const user_own_list_content = document.getElementsByClassName("user_own_list_content")[0];

            const my_list_explain_arrow = document.createElement("img");
            ...
            const my_list_explain = document.createElement("p");
            ...
            if (response_data.length > 0) {
                ...
                for (let i = 0; i < response_data.length; i++) {
                    ...
                    const user_own_list_content = document.getElementsByClassName("user_own_list_content")[0];
                    const user_every_list_container = document.createElement("div");
                    user_every_list_container.className = "user_every_list_container";
                    user_own_list_content.insertBefore(
                        user_every_list_container,
                        user_own_list_content.childNodes[0]
                    );

                    const user_list_icon = document.createElement("img");
                    ...
                    const user_list_name = document.createElement("p");
...
function render_a_list_all_place_name(list) {
    ...
    ajax("post", "/api/map_list/user/place/show", list_data, function (response_data) {
        response_data = response_data.data;
        const a_list_content = document.getElementsByClassName("a_list_content")[0];

        if (response_data.length == 0) {
            const a_list_explain = document.createElement("p");
            a_list_explain.className = "a_list_explain";
            a_list_explain.innerHTML = "此清單尚未收藏地點";
            a_list_content.appendChild(a_list_explain);

            const a_list_explain_1 = document.createElement("p");
            ...
            // 製作新的 place 的內容 p
            for (let i = 0; i < response_data.length; i++) {
                const user_place_container = document.createElement("div");
                ...
                const user_delete_place = document.createElement("p");
...
function render_collection_list_or_not(list) {
    ajax("post", "/api/map_list/user/result", list, function (response_data) {
        response_data = response_data.data;
        const a_place_content = document.getElementsByClassName("a_place_content")[0];

        if (response_data.length == 0) {
            const a_place_explain = document.createElement("p");
            ...
            const a_place_explain_1 = document.createElement("p");
...
function close_a_place_slidebar() {
    ...
    if (
        document.getElementById("a_public_list").style.width == "0px" &&
        document.getElementById("a_list").style.width == "0px" &&
        document.getElementById("search_list_and_place").style.width == "0px" &&
        document.getElementById("user_own_list").style.width == "0px"
    ) {
...
function open_a_list(e) {
    ...
    const user_a_list_icon = document.getElementsByClassName("user_a_list_icon")[0];
...
function confirm_delete() {
    const remove = confirm("確定是否刪除此清單?");
    if (remove == true) {
        delete_a_list();
    }
}
...
function switch_for_search_condition() {
    const search_condition = document.getElementsByClassName("checkbox")[0];
